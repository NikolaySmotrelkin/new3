...
public class AccountManager {
    // Объявляем поле с типом интерфейс IAccountService
    IAccountService accountService;

    public AccountManager(String path, StorageType storageType) {
        // Создаем класс (в зависимости от типа хранилища) реализующий логику работы со счетами
        switch (storageType) {
            case FILE:
    ...
    public void run() {
        ...
        while (run) {
            ...
            if ("exit".equals(command)) {
                ...
                withdraw(args);
            } else if ("deposite".equals(command)) {
                deposite(args);
